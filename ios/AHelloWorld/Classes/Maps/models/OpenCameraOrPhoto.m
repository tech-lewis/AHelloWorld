...
{
    // 从相册中选取
    if ([[GLTools sharedGLTools] isPhotoLibraryAvailable]) {
...
- (void)takePhoto
{
    if ([[GLTools sharedGLTools] isCamer])
    {
        // 拍照
        if ([[GLTools sharedGLTools] isCameraAvailable] && [[GLTools sharedGLTools] doesCameraSupportTakingPhotos]) {
            UIImagePickerController *controller = [[UIImagePickerController alloc] init];
            controller.sourceType = UIImagePickerControllerSourceTypeCamera;
            if ([[GLTools sharedGLTools] isRearCameraAvailable]) {
